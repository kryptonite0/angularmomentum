...
disk.fit = function(object,
  ...
  object = disk.fit.singe_exponential_density_profile(object,plot=T,rd_fixed)

  if (iterations>0) {
    for (i in seq(iterations)) {
      object = disk.fit.center(object,F,refit=T)
      object = disk.fit.orientation(object,plot=show.details,refit=T)
      object = disk.fit.singe_exponential_density_profile(object,plot=F,rd_fixed)
      object = disk.fit.rotationcurve(object,plot=show.details,vmax)
    }
  }
  
  object = disk.fit.double_exponential_density_profile(object,plot=show.details,rd_fixed)
...
disk.fit.rotationcurve <- function(object,plot=T,vmax=300,gammaDensity=0.0) {
  ...
  v_circular_model_best = function(r) {
    # r is the DEPROJECTED galactocentric radius in pixels
...
disk.fit.singe_exponential_density_profile <- function(object,rd_fixed,plot=T,rexp_scale_factor=1.0) {
  ...
  sigma_model = function(r,para) {
    ...
    if (is.null(rd_fixed)) {rd = para[2]} # [pixel] disk scale length
    else {rd = rd_fixed}
    #print(rd)
  ...
  par1 = c(log(c(max(densityPositive,na.rm=T),rd_fixed)),0)
  out1 = optim(par1,min_fct1,hessian=FALSE)
  par1 = out1$par
  if (!is.null(rd_fixed)) {par1[2] = log(rd_fixed) }
  ...
  min_fct2 = function(p) {
    ...
    if (!is.null(object$data[["forced_rexp_fraction"]])) {
      ...
      print("forced_rexp_fraction in use")
    } else {
  ...
  par2 = out2$par
  if (!is.null(rd_fixed)) {par2[2] = log(rd_fixed)}
  ...
  object$fit$density_model$parameter = list(single_exponential_s=exp(par2[1]),
                                            ...
                                            offset=par2[3])
  #print(object$fit$density_model$parameter$single_exponential_r)
  ...
  if (plot) {
    ...
    lines(r,sigma_model(r,exp(par2[1:2]))+object$fit$density_model$parameter$offset,col='blue',lwd=1)
    lines(r,r*0+object$fit$density_model$parameter$offset,col='green',lwd=2)
    ...
    blurred_model = blurred_model/blurred_model_n
    lines(seq(ceiling(rmax)),blurred_model[2:(ceiling(rmax)+1)]+object$fit$density_model$parameter$offset,col='blue',lwd=3)
  }
  
  return(object)
}

disk.fit.double_exponential_density_profile <- function(object,rd_fixed,plot=T) {
...
merge.objects = function(master_object,slave_object,slave_weight_density=0.35,plot=F) {
  ...
  for (mode in seq(4)) { # 1=density map, 2=velocity map, 3=pixelInsideFrame
    if (mode == 1) {
      ...
      maps = slave_weight_density*slave_object$data$density 
      # the slave does not use the offset corrected data, because the density fit and offset are very uncertain due to central beam smearing
    }
    if (mode==2) {
